...
const NavItem = ({
  ...
  return <Link to={path} className={cn("flex items-center gap-2 px-4 py-3 rounded-md transition-colors", active ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50")}>
  ...
  return <div className="h-screen w-64 bg-sidebar flex flex-col border-r border-border">
      ...
      <nav className="flex-1 p-2 overflow-y-auto bg-black">
        <div className="space-y-1">
          {navItems.map(item => <NavItem key={item.path} icon={item.icon} label={item.label} path={item.path} active={currentPath === item.path} />)}
        </div>
      </nav>
    </div>;
};
